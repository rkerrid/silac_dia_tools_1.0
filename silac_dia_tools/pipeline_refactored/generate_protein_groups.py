...
# def filter_and_compute_median_ms1andprecursortranlsated(pre_df, ms1_df):     # need to work on the ratios for using both ms1 and precursor translated
#     # Merging the dataframes on 'Protein.Group' and 'Run'
#     merged_df = pd.merge(pre_df, ms1_df, on=['Protein.Group', 'Run'])

#     # Function to filter values and compute median
#     def valid_median(series):
#         valid_series = series.replace([0, np.inf, -np.inf], np.nan).dropna()
#         return valid_series.median() if len(valid_series) >= 3 else np.nan

#     # Apply the filtering function and then calculate the median
#     result = merged_df.groupby(['Protein.Group', 'Run']).agg({
#         'Precursor.Translated H/L': valid_median,
#         'Ms1.Translated H/L': valid_median,
#         'Precursor.Translated H/M': valid_median,
#         'Ms1.Translated H/M': valid_median
#     })
